...
def exploreEnvironment():
	...
	print "--------------------  %f  ---------------" % (rospy.get_time() - now)
	print "--------------------  %f  ---------------" % (rospy.Duration(myTimeLimit).to_sec())
	if rospy.get_time() - now >= rospy.Duration(myTimeLimit).to_sec():
	    print "TIme Up!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
	    scan_sub.unregister()
	    teleop_pub.unregister()
	    #exit = True

    #rospy.signal_shutdown("time up!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")

#Save initial position
def getStartPoint(odom_data):
    ...
    if getPosition:
        ...
        file = open("/home/anirudh/catkin_ws/src/ar_project/pose.txt","w")
        file.write(repr(x) + ' ' + repr(y))
        file.close()
        myTimeLimit = int(rospy.get_param('explore_time'))
	print "REEEEEEEEECCCCCCCCCIIIIIIIIIIIIEED %f" % myTimeLimit
	getPosition = False
...
if __name__ == "__main__":
    ...
    rospy.Subscriber('odom',Odometry,getStartPoint)
    exploreEnvironment()
